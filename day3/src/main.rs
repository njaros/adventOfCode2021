...
fn found_last_line_rec(v: &mut Vec<String>, idx: usize, more: bool) -> u32 {
    match v.as_slice() {
        ...
        [s] => {
            let mut res = 0u32;
            s.chars().rev().enumerate().for_each(|(pow, c)| {
                res += parse_c(c) * 2u32.pow(pow as u32)
            });
            res
        },
        _ => {
            ...
            let count = v.iter().fold(0, |res, s| {
                if s.chars().nth(idx).unwrap() == '1' {res + 1}
                else {res}
            });
            match more {
                true    => {
                    if 2 * count >= v.len() {bit_to_be = '1' }
                    else {bit_to_be = '0'}
                },
                false   => {
                    if 2 * count >= v.len() {bit_to_be = '0' }
                    else {bit_to_be = '1'}
                }
            }
        ...
        1 => {
            for (idx, line) in lines.enumerate() {
                ...
                match idx {
                    ...
                    _ => line.chars().enumerate().for_each(
                            |(idx_char, c)| counter[idx_char] += parse_c(c) as usize
                        ),
                }
            }
            counter.iter().rev().enumerate().for_each(
                |(idx, n)| {
                    if n.to_owned() > len / 2 {
